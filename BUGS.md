...
| # | 事項 | 狀態 |
|---|---|---|
| Y1 | 音效預設改為關閉（依規格），與先前「預設開」不同 | ✅ v1.1-preview 依 Gary 真機回饋改回預設開（首頁＋作答 BGM、掃描只留特效音、結果頁另一段 BGM） |
...
## v1.1-preview（真機回饋）備註

| # | 事項 | 狀態 |
|---|---|---|
| P1 | BGM 預設開：瀏覽器規定第一次點畫面後才會出聲，首次出聲時顯示「音樂已開啟，右上角可關」提示 | ✅ 自動化 A7–A10 |
| P2 | 掃描頁特效音放大 50%（僅掃描期間） | ✅ |
| P3 | 首頁在 390×664（iPhone Safari 含網址列）一屏可見「開始挑戰」，測試版標籤移到左上 | ✅ 截圖 screenshots/v1.1 |
| P4 | 作答頁回饋列與「下一題」固定占位，威比位置作答前後不變、放大到 76px | ✅ 自動量測 |
...
| # | 事項 | 狀態 |
|---|---|---|
| Q1 | 小知識卡重整後空白：結果寫入本機時尚未算出小知識與關鍵字，重整讀回就沒有。已改為先算再存，舊存檔改從答題紀錄補算 | ✅ 自動化 E3b／F0 |
| Q2 | 聲音改兩態（開＝音效＋音樂／關），預設開；iOS 靜音鍵狀態網頁抓不到，改為第一次出聲時提示一次 | ✅ A7–A10 |
| Q3 | 頂欄 Logo 連 WEiZ 商城（新分頁、沿用結果頁 UTM）；GA 事件 logo_click | ✅ A11 |
| Q4 | 首頁威比對話框 5 句輪播（4 秒）、開始鈕呼吸光暈＋流光；Z 圓標呼吸；小知識 8 則輪播（5 秒、點一下暫停）＋ LINE CTA 卡底滿版 | ✅ A12／E3d |
| Q5 | 證書：底部改「測驗日期＋weiz.com.tw」、評語區上移；預覽鈕改保存證書／曬證書(圖示)／邀請 LINE 好友挑戰，右上角 X、點外面關閉 | ✅ E2b／E3／E3c |
