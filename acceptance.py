...
async def main():
    async with async_playwright() as p:
        ...
        await pg.click('#home [data-sound]'); await pg.wait_for_timeout(150); m2 = await pg.evaluate("[soundMode,musicOn]")
        await pg.click('#home [data-sound]'); await pg.wait_for_timeout(400); m3 = await pg.evaluate("[soundMode,musicOn,AC&&AC.state]")
        saved = await pg.evaluate("JSON.parse(localStorage.getItem('weiz_age_sound'))")
        rec('A 載入', 'A9 音效鈕三態循環 音效＋音樂→關閉→音效→音效＋音樂，並記住選擇', m1==[0,False] and m2==[1,False] and m3[:2]==[2,True] and m3[2]=='running' and saved==2, f'{m1} {m2} {m3} saved={saved}')
