...
async def main():
    async with async_playwright() as p:
        ...
        rec('A 載入', 'A7 音效預設開啟（音效＋音樂）、無障礙名稱一致', st[0]==2 and not st[1] and st[2]=='音效＋音樂' and '音效＋音樂' in st[3], str(st))
        await pg.mouse.click(200, 300); await pg.wait_for_timeout(600)
        st2 = await pg.evaluate("[soundMode, musicOn, AC&&AC.state, musicKind]")
        rec('A 載入', 'A8 第一次點畫面後音樂開始（首頁 BGM）', st2[0]==2 and st2[1] and st2[2]=='running' and st2[3]=='quiz', str(st2))
        ...
        rec('A 載入', 'A9 音效鈕只有兩態 開→關→開，並記住選擇', m1==[0,False] and m2[:2]==[2,True] and m2[2]=='running' and saved==2, f'{m1} {m2} saved={saved}')
        await pg.click('#home [data-sound]'); await pg.wait_for_timeout(300); await pg.reload(); await pg.wait_for_timeout(600)
        ...
        rec('E 分享', 'E1 「分享挑戰！」開平台面板：兩平台＋手機直接分享，Threads／FB 連結帶 share_id', sheet['shown'] and sheet['label']=='分享挑戰！' and sheet['pfs']==['Threads','Facebook','手機直接分享'] and sheet['th'].startswith('https://www.threads.net/intent/post?text=') and '%26s%3D' in sheet['th'] and sheet['fb'].startswith('https://www.facebook.com/sharer/sharer.php?u=') and '%26p%3Dfacebook' in sheet['fb'], str(sheet['pfs']))
        await pg.click('#shareSheet [data-pf=copy_text]'); await pg.wait_for_timeout(300)
        clip = await pg.evaluate("navigator.clipboard.readText().catch(()=>'')")
        rec('E 分享', 'E1c 面板「複製文案」（已合併複製連結）：內容含 #a= 與 s=／p= 歸因參數', ('#a=' in clip) and ('&s=' in clip) and ('&p=copy' in clip), clip[-60:])
        ...
        btns = await pg.evaluate("[...document.querySelectorAll('#cardprev .row .btn')].map(b=>b.textContent.trim().replace(/[@f]+$/,'').trim())")
        rec('E 分享', 'E2b 證書預覽按鈕：保存證書／曬證書(2 圖示)／複製分享文案／邀請 LINE 好友挑戰，右上角 X', btns==['保存證書','曬證書','複製分享文案','邀請 LINE 好友挑戰'] and await pg.evaluate("document.querySelector('#certLineBtn').href.startsWith('https://line.me/R/share') && document.querySelector('#closeCardBtn').classList.contains('modal-x') && document.querySelectorAll('#shareCertBtn .mini-ic').length===2"), str(btns))
        ...
        rec('E 分享', 'E3 圖卡可由右上角 X 關閉', not await pg.evaluate("document.getElementById('cardprev').classList.contains('show')"))
        await pg.click('#cardBtn'); await pg.wait_for_timeout(300); await pg.mouse.click(5, 5); await pg.wait_for_timeout(200)
        rec('E 分享', 'E3c 點證書視窗外面也會關閉', not await pg.evaluate("document.getElementById('cardprev').classList.contains('show')"))
        rot = await pg.evaluate("(()=>{const n0=[...document.querySelectorAll('.tip-slide')].findIndex(e=>e.classList.contains('on')); tipGo(tipIdx+1); const n1=[...document.querySelectorAll('.tip-slide')].findIndex(e=>e.classList.contains('on')); return {n0,n1,n:document.querySelectorAll('.tip-slide').length,count:document.querySelector('#tipCount').textContent,h:document.querySelector('#tipViewport').style.height}})()")
        rec('E 分享', 'E3d 小知識輪播：可切到下一則、計數正確、依內容固定高度', rot['n1']==(rot['n0']+1)%rot['n'] and rot['count'].startswith(f"{rot['n1']+1} /") and rot['h'].endswith('px') and int(rot['h'][:-2])>80, str(rot))
        ...
        rec('F 名單', 'F0 Email 不在主流程：右下角 Z 圓標可見、小知識卡有出現、LINE OA 入口有連結', await pg.evaluate("!document.querySelector('#offerFab').hidden && document.querySelector('#tipCard').style.display!=='none' && document.querySelector('.tip-slide.on p').textContent.length>10 && document.querySelectorAll('.tip-slide').length>=2 && document.querySelector('#tipCard .oa-cta .line-logo') && document.querySelector('#oaCard').style.display!=='none' && document.querySelector('#oaCard').href==='https://lin.ee/N50PvtRt'"))
