...
async def main():
    async with async_playwright() as p:
        ...
        rec('E 分享', 'E2b 證書預覽按鈕：保存證書／曬證書(3 圖示)／複製分享文案／邀請 LINE 好友挑戰，右上角 X', btns==['保存證書','曬證書','複製分享文案','邀請 LINE 好友挑戰'] and await pg.evaluate("document.querySelector('#certLineBtn').href.startsWith('https://line.me/R/share') && document.querySelector('#closeCardBtn').classList.contains('modal-x') && document.querySelectorAll('#shareCertBtn .mini-ic').length===3"), str(btns))
